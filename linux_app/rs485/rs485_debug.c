/* RS485 调试帧解析实现：校验 magic/version/CRC 并提取 CAN 转发字段。 */
...
#define OFFSET_CRC (RS485_DEBUG_FRAME_LENGTH - RS485_DEBUG_FRAME_CRC_LENGTH)
                                        ...
                                        protocol_parsed_msg_t *out_msg)
{
    ...
    if (length != RS485_DEBUG_FRAME_LENGTH) {
        return UNIFIED_ERR_LENGTH;
    }

    if (load_u16_le(&buffer[OFFSET_MAGIC]) != RS485_DEBUG_FRAME_MAGIC) {
        return UNIFIED_ERR_PROTOCOL_HEADER;
    }

    if (buffer[OFFSET_VERSION] != RS485_DEBUG_FRAME_VERSION) {
        return UNIFIED_ERR_PROTOCOL_HEADER;
    }
