...
### 2.1 集中式状态快照收集机制
为了向 Web 服务（`put-webd`）提供各模块连通性和收发统计，项目重构并引入了统一的**集中式状态收集器 (`status_collector_t`)**。蓝牙线程无需再自行操作全局快照文件或竞争写锁：
...
- **核心行为**：
  ...
  3. 检验 `can_dlc` 长度是否符合经典 CAN 的法定边界（$\le 8$）。同时确保未携带 `UNIFIED_CAN_FLAG_FD` 等不支持的标志，在接收端即完成经典 CAN 流量的拦截。
  4. 计算 `buffer[0]` 到 `buffer[73]` 的 CRC-16 校验码，并与 `buffer[74..75]` 的小端校验码进行匹配，确保空气中数据传输未发生任何比特翻转。

#### ④ 对接集中式状态快照收集器 (`status_collector`)
- **核心行为**：
  - 大核蓝牙后台线程在处理客户端连接、帧校验以及数据转发时，不自行操作快照文件写入。
  - 线程在不同运行出口（如成功接收数据包、校验失败、套接字出错等），周期性或实时地通过框架提供的非阻塞 API（如 `status_collector_record_rx`，`status_collector_record_error` 等）将自身的统计指标上报到全局唯一的 `status_collector_t` 结构体中。
  - 主管理层框架通过 `status_collector_write_all` 线程安全地周期写出包含蓝牙模块在内的 `/run/put/status/*.json` 状态快照文件，供 Web 端 Rust 后端进行只读展示。
...
- **生命周期绑定**：在模块线程拉起、执行或停止的每一个阶段，各子线程状态将同步映射至全局状态机。当某子线程检测到外部中断或调用 `bluetooth_server_stop()` 平滑自毁时，会自动修改状态收集上下文中的模块状态为停止并说明故障原因。
- **防止野指针与崩溃**：状态收集器采用完全基于全局只读快照的设计，子线程不再拥有文件指针的所有权，所有的数据读写操作均通过安全的指针上下文传递。因此，即使某子线程意外崩溃退出，主框架的状态收集引擎仍能安全隔离故障，绝不会引用失效的局部栈空间，从根本上杜绝了悬空指针引发网关程序发生 **Segmentation Fault** 的风险。
